...
8) Now save your changes and exit the editor, Go to Git bash, type `git status` check that the `modified:   README.md` is displayed. Then type `git add .`, afterwards `git commit -m "<your name> 🍉"` and lastly `git push origin master`.
